      ...
      if (!err && cartData) {
        ...
        _data.read('tokens', token, (err, tokenData) => {
          if (!err && tokenData) {
            ...
            // Get total price
            const reducer = (accumulator, currentValue) => config.menuItems[accumulator].price + config.menuItems[currentValue].price;
            ...
            handlers._tokens.verifyToken(token, userEmail, (tokenIsValid) => {
              if (tokenIsValid) {
                ...
                const stringOrderPostData = JSON.stringify(orderPostData);
