          ...
          if (!err && tokenData) {
            ...
            handlers._tokens.verifyToken(token, userEmail, (tokenIsValid) => {
              if (tokenIsValid) {
                ...
                helpers.makeStripeTransaction(userEmail, amount, (status, responseTransaction) => {
                  if ([200, 201].indexOf(status) == -1) {        
                    callback(status, responseTransaction);
                  } else {
                    //@TODO Send email using Mailgun
                    const receiptUrl = responseTransaction.Response.receipt_url;
                    helpers.sendMailgunEmail(userEmail, receiptUrl);
                  }
                });
